...
$VaultName = "ews-backup-vault" 
$ResourceGroupName = "rg-backuprestore" 

$subscriptionObj = Get-AzSubscription
$Subscription = $subscriptionObj.Name 
...
foreach ($item in $pvtendpoints) {
    ...
    Remove-AzPrivateEndpoint -Name $pename -ResourceGroupName $ResourceGroupName -Force #remove private endpoints
}
